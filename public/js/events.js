...
if ((dateObj.getMonth()+1) < 10) globMonth = "0"+(dateObj.getMonth()+1);
else globMonth = (dateObj.getMonth()+1);
	...
	$(document).on('keypress',".tag",function(e){
		switch (e.which){
			...
			//Add new tag if enter is pressed
				...
				//Check for empty tag
				var whitePatt = new RegExp("\s");
				if (!whitePatt.test($(this).html()) && $(this).html() != ''){
					//Remove whitespace
					var newHtml = $(this).html().replace(/&nbsp/g, '');
					var newHtml = newHtml.replace(/\s/g, '');
					$(this).html(newHtml);
				}
				if ($(this).html() != ''){
